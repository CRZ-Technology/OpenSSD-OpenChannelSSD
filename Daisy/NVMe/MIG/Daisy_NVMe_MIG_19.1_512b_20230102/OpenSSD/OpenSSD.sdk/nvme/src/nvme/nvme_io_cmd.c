...
void handle_nvme_io_read(unsigned int cmdSlotTag, NVME_IO_COMMAND *nvmeIOCmd)
{
	...
	static unsigned long dramAddr = DATA_BUFFER_BASE_ADDR;
    ...
    if(dramAddr + requestedNvmeBlock * BYTES_PER_NVME_BLOCK > DATA_BUFFER_BASE_ADDR + 512*1024*1024)
    {
    	dramAddr = DATA_BUFFER_BASE_ADDR;
    }
    memcpy((void *)dramAddr, (void *)devAddr, requestedNvmeBlock * BYTES_PER_NVME_BLOCK);
    numOfNvmeBlock = 0;

    while(numOfNvmeBlock < requestedNvmeBlock)
    {
        set_auto_tx_dma(cmdSlotTag, dmaIndex, dramAddr, NVME_COMMAND_AUTO_COMPLETION_ON);

        numOfNvmeBlock++;
        dmaIndex++;
        dramAddr += BYTES_PER_NVME_BLOCK;
    }
}
...
void handle_nvme_io_write(unsigned int cmdSlotTag, NVME_IO_COMMAND *nvmeIOCmd)
{
	unsigned int requestedNvmeBlock, dmaIndex, numOfNvmeBlock;
	unsigned long long devAddr;
	static unsigned long dramAddr = DATA_BUFFER_BASE_ADDR + 512*1024*1024;
	unsigned long dramAddrOrg;
    ...
    dmaIndex = 0;
    requestedNvmeBlock = nlb + 1;
    if(dramAddr + requestedNvmeBlock * BYTES_PER_NVME_BLOCK > DATA_BUFFER_BASE_ADDR + 1024*1024*1024)
    {
        dramAddr = DATA_BUFFER_BASE_ADDR + 512*1024*1024;
    }
    dramAddrOrg = dramAddr;
    numOfNvmeBlock = 0;

    while(numOfNvmeBlock < requestedNvmeBlock)
    {
        set_auto_rx_dma(cmdSlotTag, dmaIndex, dramAddr, NVME_COMMAND_AUTO_COMPLETION_ON);
    ...
    devAddr = (unsigned long long)DDR4_BUFFER_BASE_ADDR + (unsigned long long)startLba[0] * (unsigned long long)BYTES_PER_NVME_BLOCK;
    memcpy((void *)devAddr, (void *)dramAddrOrg, requestedNvmeBlock * BYTES_PER_NVME_BLOCK);
}
