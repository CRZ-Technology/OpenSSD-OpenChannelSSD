...
void handle_nvme_io_read(unsigned int cmdSlotTag, NVME_IO_COMMAND *nvmeIOCmd)
{
    ...
    if(devAddr + requestedNvmeBlock * BYTES_PER_NVME_BLOCK > DATA_BUFFER_BASE_ADDR + 0x30000000)
    {
...
void handle_nvme_io_write(unsigned int cmdSlotTag, NVME_IO_COMMAND *nvmeIOCmd)
{
    ...
    dmaIndex = 0;
    requestedNvmeBlock = nlb + 1;
    if(devAddr + requestedNvmeBlock * BYTES_PER_NVME_BLOCK > DATA_BUFFER_BASE_ADDR + 0x3F000000)
    {
        devAddr = DATA_BUFFER_BASE_ADDR + 0x30000000;
    }
    devAddrOrg = devAddr;
    numOfNvmeBlock = 0;
