...
//		dev->prp_pool = malloc(nprps << 3);
        dev->prp_pool = (long unsigned int *)NVME_PRP_ADDR;
...
static struct nvme_queue *nvme_alloc_queue(struct nvme_dev *dev,
					   int qid, int depth)
{
    static u64 completion_q_addr = COMPLETION_QUEUE_ADDR;
    static u64 submission_q_addr = SUBMISSION_QUEUE_ADDR;
...
{
	struct nvme_id_ctrl *ctrl = (struct nvme_id_ctrl *)IDENTIFY_NS_BUFF_ADDR;
	int ret;
...
static u64 nvme_bar[2];
void set_nvme_bar(u64 *bar)
{
    memcpy(nvme_bar, bar, sizeof(nvme_bar));
}

u64 get_nvme_bar(int dev_num)
{
	return nvme_bar[dev_num];
}
...
static int nvme_blk_probe(struct udevice *udev)
{
	...
	u8 flbas;
	struct nvme_id_ns *id = (struct nvme_id_ns *)IDENTIFY_NS_BUFF_ADDR;
...
static int nvme_probe(struct udevice *udev)
{
	...
	ndev->bar = (struct nvme_bar *)get_nvme_bar(dev_num++);
...
//	ndev->prp_pool = malloc(MAX_PRP_POOL);
	ndev->prp_pool = (long unsigned int *)NVME_PRP_ADDR;
...
U_BOOT_PCI_DEVICE(nvme2, nvme_supported);
