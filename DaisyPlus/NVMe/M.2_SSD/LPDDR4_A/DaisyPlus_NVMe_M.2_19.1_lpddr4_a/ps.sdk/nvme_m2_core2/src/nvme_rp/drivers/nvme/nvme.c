...
static const int prp_addr = NVME_PRP_CORE2_ADDR;
...
//		dev->prp_pool = malloc(nprps << 3);
        dev->prp_pool = (long unsigned int *)prp_addr;

		if (!dev->prp_pool) {
			xil_printf("Error: malloc prp_pool fail\n");
			return -ENOMEM;
		}
				...
				u32 *result, unsigned timeout)
{
	...
	for (;;) {
		...
		if ((status & 0x01) == phase)
			break;
		usleep(1);
		if (timeout_us > 0 && (++start_time >  timeout_us))
			return -ETIMEDOUT;
	}

	status >>= 1;
	if (status) {
		xil_printf("ERROR: status = %x, phase = %d, head = %d\r\n",
		       status, phase, head);
		status = 0;
...
static const u64 completionQueueAddr = COMPLETION_QUEUE_CORE2_ADDR;
static const u64 submissionQueueAddr = SUBMISSION_QUEUE_CORE2_ADDR;
static struct nvme_queue *nvme_alloc_queue(struct nvme_dev *dev,
					   int qid, int depth)
{
    static u64 completion_q_addr = completionQueueAddr;
    static u64 submission_q_addr = submissionQueueAddr;
...
//	nvmeq->cqes = (struct nvme_completion *)((u64)COMPLETION_QUEUE_ADDR + QUEUE_SIZE * qid);
	nvmeq->cqes = (struct nvme_completion *)completion_q_addr;
	completion_q_addr += QUEUE_SIZE;

	if (!nvmeq->cqes)
		goto free_nvmeq;
	memset((void *)nvmeq->cqes, 0, NVME_CQ_SIZE(depth));

//	nvmeq->sq_cmds = (struct nvme_command *)((u64)SUBMISSION_QUEUE_ADDR + QUEUE_SIZE * qid);
	nvmeq->sq_cmds = (struct nvme_command *)submission_q_addr;
	submission_q_addr += QUEUE_SIZE ;
...
static int nvme_configure_admin_queue(struct nvme_dev *dev)
{
	...
	if (page_shift < dev_page_min) {
		xil_printf("Device minimum page size (%u) too large for host (%u)\n",
		      1 << dev_page_min, 1 << page_shift);
		return -ENODEV;
	}

	if (page_shift > dev_page_max) {
		xil_printf("Device maximum page size (%u) smaller than host (%u)\n",
...
static const int identify_addr = IDENTIFY_NS_BUFF_CORE2_ADDR;
static int nvme_get_info_from_identify(struct nvme_dev *dev)
{
	struct nvme_id_ctrl *ctrl = (struct nvme_id_ctrl *)identify_addr;
	int ret;
...
static u64 nvme_bar;
void set_nvme_bar(u64 bar)
{
    nvme_bar = bar;
}

u64 get_nvme_bar(void)
{
	return nvme_bar;
}

int nvme_scan_namespace(u64 bar)
{
	struct uclass *uc;
	struct udevice *dev;
	int ret;
...
static int nvme_blk_probe(struct udevice *udev)
{
	...
	u8 flbas;
	struct nvme_id_ns *id = (struct nvme_id_ns *)identify_addr;
...
static int nvme_probe(struct udevice *udev)
{
	...
	ndev->bar = (struct nvme_bar *)get_nvme_bar();
	if (readl(&ndev->bar->csts) == -1) {
		ret = -ENODEV;
		xil_printf("Error: %s: Out of memory!\n", udev->name);
		goto free_nvme;
	}
...
//	ndev->prp_pool = malloc(MAX_PRP_POOL);
	ndev->prp_pool = (long unsigned int *)prp_addr;
