...
static float result[4][8][512][512];
int test_readwrite()
{
	int loops = 5;
	int err;
...
//						for (int dqsDelay = 0; dqsDelay < 512; dqsDelay+=4)
						{
    						int dqsDelay = 511;
							nfc_set_dqs_delay(ch, dqsDelay);

//							for (int dqDelay = 0; dqDelay < 512; dqDelay+=4)
							{
    							int dqDelay = 0;
								...
								result[ch][way][dqsDelay][dqDelay] = (float)errSum / succs;
							}
						}
					}
				}
