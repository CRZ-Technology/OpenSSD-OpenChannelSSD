...
{
	unsigned int rowAddr, lun, tempBlockNo, tempPageNo, dieNo;
	...
	switch(dieStateTablePtr->dieState[chNo][wayNo].dieState)
	{
		...
		case DIE_STATE_EXE:
			...
			else if(reqStatus == REQ_STATUS_FAIL)
			{
				...
				//xil_printf("DIE_STATE_EXE Request %x Fail - ch %x way %x rowAddr %x / completion %x statusReport %x errorInfo0 %x errorInfo1 %x\r\n",
				//		reqPoolPtr->reqPool[reqSlotTag].reqCode, chNo, wayNo, GenerateNandRowAddr(reqSlotTag),completeFlagTablePtr->completeFlag[chNo][wayNo],statusReportTablePtr->statusReport[chNo][wayNo], eccErrorInfoTablePtr->errorInfo[chNo][wayNo][0], eccErrorInfoTablePtr->errorInfo[chNo][wayNo][1] );
				if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_READ)
				{
				...
				PRINT_PPA(0, &(reqPoolPtr->reqPool[reqSlotTag].ppa));
			...
			{
				//xil_printf("REQ_STATUS_WARNING Request %x- ch %x way %x rowAddr %x / completion %x statusReport %x errorInfo0 %x errorInfo1 %x\r\n",
				//		reqPoolPtr->reqPool[reqSlotTag].reqCode, chNo, wayNo, GenerateNandRowAddr(reqSlotTag),completeFlagTablePtr->completeFlag[chNo][wayNo],statusReportTablePtr->statusReport[chNo][wayNo], eccErrorInfoTablePtr->errorInfo[chNo][wayNo][0], eccErrorInfoTablePtr->errorInfo[chNo][wayNo][1] );
				xil_printf("ECC Uncorrectable Soon on ");
				PRINT_PPA(0, &(reqPoolPtr->reqPool[reqSlotTag].ppa));
