...
(
    ...
    always @ (*)
        ...
        State_NCmdWrite2: // col 1
            rNextState <= (wIsReadColumnEnhanced)?State_NCmdWriteRow0:State_NCmdWrite3;
    ...
    always @ (*)
        if (iReset)
            ...
            case (rCurState)
            State_NCmdIssue:
                rPMLength <= (wIsReadColumnEnhanced)?16'd6:16'd3; // 1 cmd + 2 cols + 3 rows + 1 cmd or 1 cmd + 2 cols + 1 cmd
            State_NTimer1Issue:
